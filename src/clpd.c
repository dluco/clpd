/*
 * clpd - clipboard daemon
 *
 * Copyright (C) 2014 David Luco <davidjluco@gmail.com>
 *
 * forked from parcellite, which is
 * Copyright (C) 2007-2008 Xyhthyx <xyhthyx@gmail.com>
 *
...
static void print_clipboard_text(GdkAtom selection)
{
	GtkClipboard *clipboard;
	char *temp;

	clipboard = gtk_clipboard_get(selection);
	temp = gtk_clipboard_wait_for_text(clipboard);
	if (temp) {
		printf("%s", temp);
	}
	g_free(temp);
...
static void daemon_check()
{
	...
	if ((primary_temp == NULL) && (primary_text != NULL)) {
		...
		GdkAtom *targets;
		gboolean contents =
			gtk_clipboard_wait_for_targets(primary, &targets, &count);
		g_free(targets);
...
{
	/* FIXME: error on trying to remove source
	   if (timeout_id != 0)
	...
	timeout_id = g_timeout_add_full(G_PRIORITY_LOW,
									...
									NULL, (GDestroyNotify) reset_daemon);
}

/* Initializes daemon mode */
static void init_daemon()
{
	/* Create primary and clipboard and connect signals */
...
static Options *parse_options(int argc, char *argv[])
{
	...
	GOptionEntry main_entries[] = {
		{ "primary", 'p', 0, G_OPTION_ARG_NONE, &opts->primary, "Print primary contents", NULL },
		{ "clipboard", 'c', 0, G_OPTION_ARG_NONE, &opts->clipboard, "Print clipboard contents", NULL },
		{ "version", 'v', 0, G_OPTION_ARG_NONE, &opts->version, "Display version info", NULL },
		{ NULL }
	};

	/* Option parsing */
	GOptionContext *context = g_option_context_new("- clipboard daemon");
...
int main(int argc, char *argv[])
{
	...
	if (opts->primary) {
		print_clipboard_text(GDK_SELECTION_PRIMARY);
	} else if (opts->clipboard) {
		print_clipboard_text(GDK_SELECTION_CLIPBOARD);
	} else {
		init_daemon();
